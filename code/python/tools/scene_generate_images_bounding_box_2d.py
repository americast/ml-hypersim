...
for in_filename in in_filenames:
    ...
    out_rgb_bb_jpg_file   = os.path.join(out_preview_dir, in_file_root + ".bb_" + args.bounding_box_type + "_2d.jpg")
    ...
    for fi in range(num_fragments):
        ...
        if str(color) not in colours_dict_x:
            colours_dict_x[str(color)] = []
        colours_dict_x[str(color)].extend([p1_screen[0], p2_screen[0]])
        if str(color) not in colours_dict_y:
            colours_dict_y[str(color)] = []
        colours_dict_y[str(color)].extend([p1_screen[1], p2_screen[1]])
        # if fi == 999:
        #     # pu.db
        #     break
    thetas = []
    x_all, y_all = [], []
    for colour in colours_dict_x:
        x_s = colours_dict_x[colour]
        y_s = colours_dict_y[colour]
        x_all.append(x_s)
        y_all.append(y_s)
        x_min = min(x_s)
        x_max = max(x_s)
        y_min = min(y_s)
        y_max = max(y_s)
        y_at_x_min = y_s[np.argmin(x_s)]
        y_at_x_max = y_s[np.argmax(x_s)]

        tan_theta = (y_at_x_max - y_at_x_min) / (x_max - x_min)
        theta = np.arctan(tan_theta)
        thetas.append(theta)

        # pu.db
        colour_here = colour[1:-1].split(" ")
        colour_here = [int(x.strip()) for x in colour_here if x !=""]
        # draw.rectangle([x_min,y_min,x_max,y_max], outline=tuple(colour_here), width=lw)

    avg_theta = np.mean(thetas)
    x_mean, y_mean = img_pil.size
    x_mean/=2; y_mean/=2
    img_pil_rotated = img_pil.rotate(np.fabs(avg_theta * 180 / np.pi), PIL.Image.NEAREST, expand = 1)
    draw = PIL.ImageDraw.Draw(img_pil_rotated)
    x_mean_rotated, y_mean_rotated = img_pil_rotated.size
    x_mean_rotated/=2; y_mean_rotated/=2
    # pu.db
    colours_dict_x_rotated, colours_dict_y_rotated = {}, {}
    for colour in colours_dict_x:
        x_s = colours_dict_x[colour]
        y_s = colours_dict_y[colour]
        x_s, y_s = np.array(x_s), np.array(y_s)
        x_s -= x_mean; y_s -= y_mean
        x1_s = np.cos(avg_theta) * x_s - np.sin(avg_theta) * y_s
        y1_s = np.sin(avg_theta) * x_s + np.cos(avg_theta) * y_s
        x1_s += x_mean_rotated
        y1_s += y_mean_rotated
        colours_dict_x_rotated[colour] = x1_s
        colours_dict_y_rotated[colour] = y1_s
        # x1_s 
    for colour in colours_dict_x_rotated:
        x1_s = colours_dict_x_rotated[colour]
        y1_s = colours_dict_y_rotated[colour]
        x_min = min(x1_s)
        x_max = max(x1_s)
        y_min = min(y1_s)
        y_max = max(y1_s)
    ...
    img_pil_rotated.save(out_rgb_bb_jpg_file)
    # pu.db
